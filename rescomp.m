function [res] = rescomp(hi, Gi, hri, hmi, Gmi, hrmi, hpi, Gpi, hrpi, ri, hipre, Gipre, hripre, hmmi, hppi, equ)
%%%
...
%%%
  ...
  spi = (sdimpi-sinf)/(so-sinf); %%% Surface tension (dimensionless) (i)
...
%%%
%%% Film thickness
  hder3 = (hppi-2.0*hpi+2.0*hmi-hmmi)/(2.0*dr^3); 
  hder4 = (hppi-4.0*hpi+6.0*hi-4.0*hmi+hmmi)/(dr^4);
%%% Film thickness
...
%%% Viscosity
  ...
  if (equ == 1) %%% Film thickness Evolution
    ...
    res = resh;
  elseif (equ == 2) %%% Surfactant concentration
